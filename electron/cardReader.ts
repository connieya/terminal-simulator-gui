...
interface PCSCLiteInstance {
  readers: Record<string, CardReaderRef>;
  on(event: "reader", cb: (reader: CardReaderRef) => void): void;
...
/** CardReader (connect, disconnect, transmit, on('status'), name 등) */
interface CardReaderRef {
  name: string;
  connect(
  ...
  on(
    event: "status",
    cb: (status: { state: number; atr?: Buffer }) => void,
  ): void;
...
let pcscInstance: PCSCLiteInstance | null = null;
/** 현재 선택해 연결한 리더 이름. 연결 해제 시 null */
let connectedReaderName: string | null = null;
/** 연결 시 확정된 프로토콜 (T=0/T=1). transmit 시 사용 */
let connectedProtocol: number | null = null;
...
export function disconnectReader(): Promise<void> {
  ...
  return new Promise((resolve) => {
    const disposition =
      reader.SCARD_LEAVE_CARD ?? reader.SCARD_UNPOWER_CARD ?? 0;
...
export function transmitApdu(
  ...
  const protocol = connectedProtocol as number;
  return new Promise((resolve, reject) => {
 ...
 */
export async function ensureConnected(): Promise<{
  success: boolean;
  readerName?: string;
  error?: string;
}> {
...
export function waitForCardAndConnect(options?: {
  ...
  return new Promise((resolve) => {
    ...
    const run = async () => {
      ...
      const onStatus = (status: { state: number }) => {
        if ((status.state & SCARD_STATE_PRESENT) === 0) return;
        removeStatusListener?.();
      ...
      timeoutId = setTimeout(() => {
        ...
        finish({
          success: false,
          error:
            "카드 태그 대기 시간이 초과되었습니다. 리더에 카드를 태그해 주세요.",
        });
      }, timeoutMs);
    };
 ...
 */
export function closePcsc(): void {
  if (pcscInstance) {
    try {
      pcscInstance.close();
    } catch {
 ...
 */
export function onReaderStatus(
  readerName: string,
