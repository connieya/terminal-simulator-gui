...
function buildSelectByNameApdu(name) {
...
function buildPdolValueForItem(item) {
    const { tag, length } = item;
    const t1 = tag[0];
    const t2 = tag.length >= 2 ? tag[1] : null;
    const pad = (n) => Buffer.alloc(n, 0);
    if (t1 === 0x9f && t2 === 0x66)
        return pad(length); // Optional
    if (t1 === 0x9f && t2 === 0x02)
        return pad(length); // Amount Authorized (BCD) 0
    if (t1 === 0x9f && t2 === 0x03)
        return pad(length); // Amount Other
    if (t1 === 0x9f && t2 === 0x1a) {
        const b = pad(length);
        b[0] = 0x04;
        if (length >= 2)
    ...
    if (t1 === 0x9a) {
        ...
        if (length === 3)
            return Buffer.from([bcd(yy), bcd(mm), bcd(dd)]); // YYMMdd
        if (length >= 6) {
    ...
    if (t1 === 0x9c)
        return Buffer.from([0x00]); // Transaction Type: purchase
    if (t1 === 0x9f && t2 === 0x37) {
    ...
    if (t1 === 0x5f && t2 === 0x2d) {
        ...
        "koen".split("").forEach((c, i) => { if (i < length)
            lang[i] = c.charCodeAt(0); });
        return lang;
    }
    if (t1 === 0x9f && t2 === 0x11)
...
function extractAflFromGpoResponse(gpoResp) {
    const entries = [];
    let tag94 = findTlv1(gpoResp, 0x94);
    if (!tag94) {
        const tag80 = findTlv1(gpoResp, 0x80);
        if (tag80)
            tag94 = findTlv1(tag80, 0x94);
    }
    if (!tag94) {
        const tag77 = findTlv1(gpoResp, 0x77);
        if (tag77)
            tag94 = findTlv1(tag77, 0x94);
    }
    if (!tag94 || tag94.length < 4)
        return entries;
...
function collectTagsForIcc(records) {
    const collected = [];
    const seen = new Set();
    function parseTlv(data, start) {
    ...
    function advanceOffset(data, start) {
...
function buildTag55(parts) {
    const body = Buffer.concat(parts);
    if (body.length >= 128)
        throw new Error("ICC Data length >= 128 not implemented");
    return Buffer.concat([Buffer.from([0x55, body.length]), body]);
}
export async function runEmvContactless(transmitApdu, log) {
    const allRecords = [];
    try {
