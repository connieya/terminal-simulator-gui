...
declare global {
  interface Window {
    electronAPI: {
      ...
      cardReader: {
        waitForCardAndConnect: (options?: { timeoutMs?: number }) => Promise<{
          success: boolean;
          readerName?: string;
          error?: string;
        }>;
        runEmvTransaction: () => Promise<{
          success: boolean;
          iccDataHex?: string;
          error?: string;
        }>;
      };
      /** Main에서 보내는 로그 수신 (EMV 등) - 반환값으로 구독 해제 */
      onTcpLogAdd: (
...
export class TcpClientService {
  ...
  async connect(config?: TcpConnectionConfig): Promise<void> {
    console.log(
      "[TcpClient] connect called, window.electronAPI:",
      window.electronAPI,
    );
    if (!window.electronAPI) {
      ...
      this.addLog(
        "error",
        "Electron API를 찾을 수 없습니다. Electron 환경에서 실행하세요.",
      );
      throw new Error(
        "Electron API is not available. Please run this application in Electron, not in a web browser.",
      );
    }
    this.addLog(
      "info",
      `TCP 연결 시도: ${config?.host ?? "localhost"}:${config?.port ?? 9999}`,
    );
  ...
  async disconnect(): Promise<void> {
    ...
    if (!result.success) {
      this.addLog(
        "error",
        `TCP 연결 해제 실패: ${result.error || "Unknown error"}`,
      );
   ...
   * 카드 탭 트리거
   */
  async tapCard(cardData?: {
    type?: "transport" | "credit" | "debit";
    data?: string;
